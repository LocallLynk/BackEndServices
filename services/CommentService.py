...
def add_comment(comment):
    ...
    post = db.session.query(Post).filter(Post.post_id == Comment.post_id).first()
    if not post:
...
def get_comment_by_id(comment_id):
    return db.session.query(Comment).filter(Comment.comment_id == comment_id).first()
...
def update_comment(comment_id, comment):
    db.session.query(Comment).filter(Comment.comment_id == comment_id).update(comment, synchronize_session="fetch")
    db.session.commit()
...
def delete_comment(comment_id):
    ...
    db.session.query(Comment).filter(Comment.comment_id == comment_id).delete()
    db.session.commit()
